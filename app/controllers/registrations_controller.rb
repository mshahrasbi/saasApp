    ...
    def create
        ...
        if !::Milia.use_recaptcha  ||  verify_recaptcha
            ...
            Tenant.transaction  do
                ...
                if @tenant.errors.empty?   # tenant created
                    ...
                    if @tenant.plan == 'premium'
                        @payment = Payment.new({
                            ...
                            token: user_params[:payment]["token"],
                            tenant: @tenant
                        })
                        
                        flash[:error] = "Please check registeration errors" unless @payment.valid?

                        begin
                            @payment.process_payment
                            @payment.save
                        rescue Exception => e
                            flash[:error] = e.full_messages
                            @tenant.destroy
                ...
                if flash[:error].blank? || flash[:error].empty? # means payment successful
